...
onload = function () {
    if (!device.mobile()) {
        document.getElementById("title1").innerHTML += `<p>Desktop click here <button id="btn4">Start</button> or press Ctrl+Shift+Z<p>`;
...
function dm(event) {
    ...
    document.getElementById("sens").innerHTML =
        `${accx.toFixed(2)}, ${accy.toFixed(2)}, ${accz.toFixed(2)}<br>Max: ${amx.toFixed(2)}, ${amy.toFixed(2)}, ${amz.toFixed(2)}`;
...
clearCacheButton.addEventListener("click", function () {
    if (checkIfVisible() && confirm("Are you sure you want to clear audio cache?")) {
        clearAudioCache();
        alert("Cleared!");
        cacheOk.innerHTML = "💬";
    }
...
accuracy.addEventListener("input", function () {
    ...
    let msg = accuracy >= 20 ? "Hold the device firmly!" : "Place it flat on the table";
    document.getElementById("lmdv").innerHTML =
        `[${Number(igiari_accuracy).toFixed(1)}]&nbsp;&nbsp;${msg}`;
});
...
document.getElementById("obje-div").addEventListener("click", function () {
    if (new Date().getTime() - touchtime < 300) {
        ...
        if ([...dashboard.classList].includes("hide2")) {
...
function objection() {
    if (!inob) {
        try { clearTimeout(timer1); } catch (e) { }
...
document.body.addEventListener("keydown", function (event) {
    if (event.ctrlKey && event.shiftKey && event.keyCode == 90) {
        ...
        if (!mousemode) {
            clearCacheButton.innerHTML = "Stop";
            window.mousemode = true;
            document.body.classList.add("hidecur");
            document.body.addEventListener("mousemove", objection);
        } else {
            clearCacheButton.innerHTML = "Start";
            window.mousemode = false;
            document.body.classList.remove("hidecur");
            document.body.removeEventListener("mousemove", objection);
        }
